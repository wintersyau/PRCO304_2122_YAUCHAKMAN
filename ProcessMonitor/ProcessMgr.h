...
BOOLEAN GetPathByFileObject(PFILE_OBJECT FileObject, WCHAR* wzPath)
{
	...
	__try
	{
		...
		{
			if (NT_SUCCESS(IoQueryFileDosDeviceName(FileObject, &ObjectNameInformation)))   //Note that the memory should be released after the function is called!
			{
					...
					if (Buffer)
					{
						...
						if (NT_SUCCESS(Status))
						{
							...
							if (Temp->Name.Buffer != NULL)
							{
								...
								{
									// If it exists in the form of "\ \ device \ \ harddiskvolume", then query its volume name.
								...
								{
									//If it doesn't start with "\ \ device \ \ harddiskvolume", copy the name directly.
